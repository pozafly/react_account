...
const ContractSearchHead = ({setContractNo}) => {
    ...
    const onRowSelected = (event) => {
        console.log(event.node.selected + event.data.contractNo);
        if(event.node.selected){
            setContractNo(event.data.contractNo);
        }
    }
    ...
    const fetchPayStepCodeList = () => {
        ...
        if(!fromDate && !toDate){
            ...
            setUrl('logi/business/findContractList?fromDate='+fromDate+'&toDate='+toDate);
            refetch();
        }
    };
    const positionColumnDefs = [
        ...
        { headerName: "수주일자", field: "contractDate", width: 200 ,hide:true },
        { headerName: "수주상태", field: "contractStatus", width: 200 ,hide:true },
        { headerName: "견적담당자", field: "personCodeInCharge", width: 200 },
        { headerName: "수주요청자", field: "contractRequester", editable: true, width: 200, hide:true },
        { headerName: "납품완료상태", field: "deliveryResultStatus", width: 200 },
